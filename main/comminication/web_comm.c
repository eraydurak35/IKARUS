...
radio_control_t *radio_ptr = NULL;
states_t *states_ptr = NULL;
flight_t *flight_ptr = NULL;
telemetry_small_integer_t *telemetry_ptr = NULL;
...
static esp_err_t send_recv_handler(httpd_req_t *req)
{
    ...
    // -120,120
    radio_ptr->channel[RC_YAW_CH] = (int8_t)(ws_pkt.payload[0]); // yaw
    radio_ptr->channel[RC_THROTTLE_CH] = (int8_t)(ws_pkt.payload[1]); // thr
    radio_ptr->channel[RC_ROLL_CH] = (int8_t)(ws_pkt.payload[2]); // roll
    radio_ptr->channel[RC_PITCH_CH] = (int8_t)(ws_pkt.payload[3]); // pitch

    radio_ptr->channel[RC_CH_5] = (uint8_t)(ws_pkt.payload[4]); // headless_button
    radio_ptr->channel[RC_CH_6] = (uint8_t)(ws_pkt.payload[5]); // flip_button
    radio_ptr->channel[RC_CH_7] = (uint8_t)(ws_pkt.payload[6]); // alt_hold_button

    //printf("%d, %d, %d, %d, %d, %d, %d\n", radio_ptr->channel[RC_YAW_CH], radio_ptr->channel[RC_THROTTLE_CH], radio_ptr->channel[RC_ROLL_CH], radio_ptr->channel[RC_PITCH_CH], radio_ptr->channel[RC_CH_5], radio_ptr->channel[RC_CH_6], radio_ptr->channel[RC_CH_7]);
    ...
    if (err == ESP_OK)
    {
        telemetry_ptr->rssi = sta_list.sta[0].rssi;
    }

    telemetry_ptr->battery_voltage = (uint8_t)(flight_ptr->battery_voltage * 50.0f);
...
/*  telemetry.is_flip_on = telem_small_p->is_flip_on;
    ...
    telemetry.is_alt_hold_on = telem_small_p->is_alt_hold_on; */

    memcpy(buff, telemetry_ptr, sizeof(telemetry_small_integer_t));
    memset(&ws_pkt, 0, sizeof(httpd_ws_frame_t));
...
void web_comm_init(radio_control_t *radio, states_t *st, flight_t *flt, telemetry_small_integer_t *telem)
{
    radio_ptr = radio;
    states_ptr = st;
    flight_ptr = flt;
    ...
    radio_ptr->channel[0] = 0;
    radio_ptr->channel[1] = 0;
    radio_ptr->channel[2] = 0;
    radio_ptr->channel[3] = 0;
    radio_ptr->channel[4] = 0;
    radio_ptr->channel[5] = 0;
    radio_ptr->channel[6] = 0;

...
{
    uint8_t buff[21] = {0};
    i2c_read_bytes(I2C_NUM_1, BMP390_ADDR, CALIB_START_REG, buff, 21);
...
void bmp390_read_i2c(bmp390_t *bmp)
{
    ...
    i2c_read_bytes(I2C_NUM_1, BMP390_ADDR, DATA_0, buff, 6);
    parse_bmp390_data_bytes(bmp, buff);
}

void bmp390_read_spi(bmp390_t *bmp)
{
    // read 7 bytes and discard first byte
    static uint8_t buff[7] = {0};
    spi_read_bytes(bmp390_handle, DATA_0, buff, 7);
    parse_bmp390_data_bytes(bmp, buff + 1);
    baro_get_altitude_velocity(bmp);
}
...
static void baro_get_altitude_velocity(bmp390_t *baro)
{
    ...
    baro->velocity_ms = (baro->altitude_m - prev_altitude_m) / dt;
    prev_altitude_m = baro->altitude_m;
}
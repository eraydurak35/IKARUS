...
{
    float omega = 2.0f * M_PI * cf / sample_freq;
    float sn = sinf(omega);
    float cs = cosf(omega);
    float alpha = sn / 2.0f;
...
void biquad_lpf_array_init(uint8_t lenght, biquad_lpf_t *lpf, float cutoff_freq, float sample_freq)
{
    for (uint8_t i = 0; i < lenght; i++)
    {
        biquad_lpf_configure(cutoff_freq, sample_freq, lpf + i);
    }
}

...
static radio_control_t prv_radio;
static radio_control_t *radio_ptr;
...
void small_drone_control_init(radio_control_t *rad, telemetry_small_integer_t *telemetry, flight_t *flight, target_t *target, states_t *state, config_t *config)
{
    ...
    biquad_lpf_configure(80.0f, 1000.0f, &lpf_pitch_d_term);
    biquad_lpf_configure(80.0f, 1000.0f, &lpf_roll_d_term);
}
...
static void small_drone_check_flight_mode()
{
    ...
    if (radio_ptr->channel[RC_CH_6] == 1 && prv_radio.channel[RC_CH_6] == 0) telemetry_p->is_flip_on = 1;
    else if (radio_ptr->channel[RC_CH_6] == 0) telemetry_p->is_flip_on = 0;

    if (radio_ptr->channel[RC_CH_7] == 1 && telemetry_p->is_alt_hold_on == 0)
    {
        telemetry_p->is_alt_hold_on = 1;
        pid.altIout = target_p->throttle;
        target_p->altitude = state_p->altitude_m;
    }
    else if (radio_ptr->channel[RC_CH_7] == 0 && telemetry_p->is_alt_hold_on == 1) telemetry_p->is_alt_hold_on = 0;
...
static void inner_control_loop()
{
    ...
    {
        //↓↓↓↓↓↓↓↓↓↓  PITCH I CALCULATION   ↓↓↓↓↓↓↓↓↓↓
        pid.pitchIout += config_p->pitch_i * 0.000625f * (pid.errPitch + pid.errPitchPrev);  //  0.00125 = 0.5 * sampleTime

...
uint8_t accelerometer_calibration(imu_t *imu)
{
    ...
    status_led_set_brightness(data_point_counter / 10);
...
// Mnayetik sensörün kalibrasyonunu gerçekleştirir
// Kalibrasyon sürerken 0, kalibrasyon bittiğinde 1 döndürür
uint8_t magnetometer_calibration(magnetometer_t *mag)
{
    ...
    if (!init)
    {
        ...
        else if (counter < 20) status_led_set_brightness(0);
        else if (counter < 30) status_led_set_brightness(100);
        else if (counter < 40) 
        {
    ...
    else
    {
        ...
        status_led_set_brightness((counter%200) / 2);
...
// Gyroscope sensörünün kalibrasyonunu gerçekleştirir
// Kalibrasyon sürerken 0, kalibrasyon bittiğinde 1 döndürür
uint8_t gyro_calibration(imu_t *imu)
{
    static uint16_t i = 0;
    static float sum_x = 0, sum_y = 0, sum_z = 0;
    // Kalibrasyon IMU hareketsiz iken devam edebilir.
    // Hareketin büyüklüğünü hesaplamak için ölçümlerin mutlak değerini al ve topla
    float movement_vector = fabsf(imu->gyro_dps[X]) + fabsf(imu->gyro_dps[Y]) + fabsf(imu->gyro_dps[Z]);

    // Hareket büyüklüğü eşik değerin altındaysa IMU sabit diyebiliriz. Veri toplamaya devam et.
    if (movement_vector < GYR_CAL_MVMT_THRESHOLD)
    {
        // Verilerin integralini al
        sum_x += imu->gyro_dps[X];
        sum_y += imu->gyro_dps[Y];
        sum_z += imu->gyro_dps[Z];
        i++;
        // 1000 adet veri toplandıysa
        if (i == 1000)
        {
            // Ortalamasını al ve kayıklı (bias) değerini bul
    ...
    // Değilse tüm veriyi sıfırla ve yeniden başla
    else
    {
        i = 0;
        sum_x = 0;
    ...
    // Kalibrasyon süresince 0 döndür
    return 0;
}
// Kalibrasyon verilerini varsayılan değere döndürür

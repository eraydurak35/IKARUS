...
#define MAVLINK_MSG_ID_IMU 116


typedef struct __mavlink_imu_t {
 int16_t gyro_x; /*<  X gyro dps * 100*/
 int16_t gyro_y; /*<  Y gyro dps * 100*/
 int16_t gyro_z; /*<  Z gyro dps * 100*/
 int16_t accel_x; /*<  X accel ms2 * 400*/
 int16_t accel_y; /*<  Y accel ms2 * 400*/
 int16_t accel_z; /*<  Z accle ms2 * 400*/
 int16_t mag_x; /*<  X mag mgaus*/
 int16_t mag_y; /*<  Y mag mgaus*/
 int16_t mag_z; /*<  Z mag mgaus*/
 uint16_t temp_mC; /*<  IMU Temperature mC*/
} mavlink_imu_t;

#define MAVLINK_MSG_ID_IMU_LEN 20
#define MAVLINK_MSG_ID_IMU_MIN_LEN 20
#define MAVLINK_MSG_ID_116_LEN 20
#define MAVLINK_MSG_ID_116_MIN_LEN 20

#define MAVLINK_MSG_ID_IMU_CRC 253
#define MAVLINK_MSG_ID_116_CRC 253
...
#define MAVLINK_MESSAGE_INFO_IMU { \
    ...
    {  { "gyro_x", NULL, MAVLINK_TYPE_INT16_T, 0, 0, offsetof(mavlink_imu_t, gyro_x) }, \
         ...
         { "temp_mC", NULL, MAVLINK_TYPE_UINT16_T, 0, 18, offsetof(mavlink_imu_t, temp_mC) }, \
         } \
}
#else
#define MAVLINK_MESSAGE_INFO_IMU { \
 ...
 *
 * @param gyro_x  X gyro dps * 100
 * @param gyro_y  Y gyro dps * 100
 * @param gyro_z  Z gyro dps * 100
 * @param accel_x  X accel ms2 * 400
 * @param accel_y  Y accel ms2 * 400
 * @param accel_z  Z accle ms2 * 400
 * @param mag_x  X mag mgaus
 * @param mag_y  Y mag mgaus
 * @param mag_z  Z mag mgaus
 * @param temp_mC  IMU Temperature mC
 * @return length of the message in bytes (excluding serial stream start sign)
 */
static inline uint16_t mavlink_msg_imu_pack(uint8_t system_id, uint8_t component_id, mavlink_message_t* msg,
                               int16_t gyro_x, int16_t gyro_y, int16_t gyro_z, int16_t accel_x, int16_t accel_y, int16_t accel_z, int16_t mag_x, int16_t mag_y, int16_t mag_z, uint16_t temp_mC)
{
    ...
    _mav_put_uint16_t(buf, 18, temp_mC);
...
{
    return mavlink_msg_imu_pack(system_id, component_id, msg, imu->gyro_x, imu->gyro_y, imu->gyro_z, imu->accel_x, imu->accel_y, imu->accel_z, imu->mag_x, imu->mag_y, imu->mag_z, imu->temp_mC);
}
...
{
    return mavlink_msg_imu_pack_chan(system_id, component_id, chan, msg, imu->gyro_x, imu->gyro_y, imu->gyro_z, imu->accel_x, imu->accel_y, imu->accel_z, imu->mag_x, imu->mag_y, imu->mag_z, imu->temp_mC);
}
...
{
    return mavlink_msg_imu_pack_status(system_id, component_id, _status, msg,  imu->gyro_x, imu->gyro_y, imu->gyro_z, imu->accel_x, imu->accel_y, imu->accel_z, imu->mag_x, imu->mag_y, imu->mag_z, imu->temp_mC);
}
...
static inline void mavlink_msg_imu_send(mavlink_channel_t chan, int16_t gyro_x, int16_t gyro_y, int16_t gyro_z, int16_t accel_x, int16_t accel_y, int16_t accel_z, int16_t mag_x, int16_t mag_y, int16_t mag_z, uint16_t temp_mC)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    char buf[MAVLINK_MSG_ID_IMU_LEN];
    _mav_put_int16_t(buf, 0, gyro_x);
...
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    mavlink_msg_imu_send(chan, imu->gyro_x, imu->gyro_y, imu->gyro_z, imu->accel_x, imu->accel_y, imu->accel_z, imu->mag_x, imu->mag_y, imu->mag_z, imu->temp_mC);
#else
 ...
 */
static inline void mavlink_msg_imu_send_buf(mavlink_message_t *msgbuf, mavlink_channel_t chan,  int16_t gyro_x, int16_t gyro_y, int16_t gyro_z, int16_t accel_x, int16_t accel_y, int16_t accel_z, int16_t mag_x, int16_t mag_y, int16_t mag_z, uint16_t temp_mC)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
...
/**
 * @brief Get field temp_mC from imu message
 *
 * @return  IMU Temperature mC
 */
static inline uint16_t mavlink_msg_imu_get_temp_mC(const mavlink_message_t* msg)
{
...
static inline void mavlink_msg_imu_decode(const mavlink_message_t* msg, mavlink_imu_t* imu)
{
    ...
    imu->temp_mC = mavlink_msg_imu_get_temp_mC(msg);
#else
